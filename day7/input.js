const string = `posh crimson bags contain 2 mirrored tan bags, 1 faded red bag, 1 striped gray bag.
...
module.exports = string.split("\n").reduce((map, line) => {
  let [container, containedBags] = line.split(" bags contain ");
  containedBags = containedBags.split(", ").map(bag => {
    if (bag === "no other bags.") {
      return false;
    }

    let count = parseInt(bag[0]);
    let [bagColor, ] = bag.substr(2).split(" bag");
    return { count, bagColor };
  });

  map[container] = containedBags.filter(val => val);
  return map;
}, {});
