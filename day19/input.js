...
bbbbbaabaaabaaabaabbaabbbbaababa`.split("\n\n");

let rules = string[0].split("\n").reduce((map, line) => {
  let [order, chars] = line.split(": ");
  let rulesList = chars
    .split(" | ")
    .map(chars => chars.split(" ").map(char => char.match(/\d/) ? Number(char) : char.replace(/"/g, "")));

  map.set(parseInt(order), rulesList);
  return map;
}, new Map());

module.exports = {
  rules,
  messages: string[1].split("\n")
};
